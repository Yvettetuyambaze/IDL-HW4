...
class ASRDataset(Dataset):
    def __init__(
        ...
        if self.partition != "test-clean":
            ...
            # Verify pairwise alignment: each fbank and text file must share the same basename
            # (e.g. "103-1240-0000.npy" in fbank/ must pair with "103-1240-0000.npy" in text/)
            for fbank_path, text_path in zip(self.fbank_files, self.text_files):
                fbank_name = os.path.basename(fbank_path)
                text_name  = os.path.basename(text_path)
                if fbank_name != text_name:
                    raise ValueError(
                        f"FBANK file {fbank_path} and TRANSCRIPT file {text_path} are misaligned."
                    )
        else:
