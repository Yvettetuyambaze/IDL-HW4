...
class ASRTrainer(BaseTrainer):
    ...
    def __init__(self, model, tokenizer, config, run_name, config_file, device=None):
        ...
        # Initialize CE loss
        # How would you set the ignore_index? Use pad token id.
    ...
    def _train_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.autocast(device_type=self.device, dtype=torch.float16):
                ...
                # Calculate CE loss
                # seq_out: (batch, seq_len, vocab), targets_golden: (batch, seq_len)
                ce_loss = self.ce_criterion(seq_out.view(-1, seq_out.size(-1)), targets_golden.view(-1))
                
                # Calculate CTC loss if needed
                if self.ctc_weight > 0:
                    # ctc_inputs['log_probs']: (seq_len, batch, vocab)
                    # ctc_inputs['lengths']: (batch,)
                    # targets_golden: (batch, seq_len), we need to remove padding for CTC
                    # Use the original transcript lengths (without SOS/EOS?) CTC expects target lengths without special tokens.
                    # But we have tokenized transcript (without SOS/EOS) stored? Actually we have tokenized list in dataset.
                    # Simpler: use the lengths of golden transcripts minus EOS? But CTC expects target lengths as number of tokens (excluding padding).
                    # We can compute target_lengths_ctc = transcript_lengths - 1 (because golden has EOS appended).
                    target_lengths_ctc = transcript_lengths - 1  # remove EOS token
                    # Also we need to strip the last token (EOS) from targets_golden for CTC.
                    targets_ctc = targets_golden[:, :-1]  # remove EOS
                    # Compute CTC loss
                    ctc_loss = self.ctc_criterion(
                        ctc_inputs['log_probs'],  # (T, N, C)
                        targets_ctc,              # (N, S)
                        ctc_inputs['lengths'],    # (N,)
                        target_lengths_ctc        # (N,)
                    )
    ...
    def _validate_epoch(self, dataloader):
        ...
        """
        # Use recognize to get transcriptions
        recognition_config = {
            'num_batches': None,
            'beam_width': 1,
            'temperature': 1.0,
