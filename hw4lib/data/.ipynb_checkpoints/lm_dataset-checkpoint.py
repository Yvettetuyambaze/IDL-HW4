...
class LMDataset(Dataset):
    ...
    def __init__(
        ...
        for file in tqdm(self.text_files):
            # Load the transcript
            transcript = np.load(file).item()  # each .npy contains a string
    ...
    def collate_fn(self, batch: List[Tuple[torch.LongTensor, torch.LongTensor]]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        lengths = torch.tensor([len(t) for t in shifted_transcripts], dtype=torch.long)
    ...
    def sample_prompts(self, num_samples: int, prompt_length: int, seed: int = None) -> Tuple[torch.LongTensor, List[torch.LongTensor]]:
        ...
        DO NOT MODIFY signature, but fix tensor/list conversion.
        ...
        while len(prompts) < num_samples and attempts < max_attempts:
            ...
            # FIX: convert tensors to lists before concatenation
            prompts.append(torch.LongTensor([self.sos_token] + prompt_tokens.tolist()))
            originals.append(torch.LongTensor(tokens.tolist() + [self.eos_token]))
