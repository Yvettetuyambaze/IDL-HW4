...
## -------------------------------------------------------------------------------------------------
## Encoder-Decoder Transformer (placeholder – not required for HW4P1)
## -------------------------------------------------------------------------------------------------
# ... existing imports and DecoderOnlyTransformer (unchanged) ...
...
class EncoderDecoderTransformer(nn.Module):
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict, dict]:
        ...
        ctc_logits = self.ctc_head(x_enc)  # (batch, seq_len, num_classes)

        # Return the encoded representation, padding mask, running attention weights, and CTC inputs (see docstring)
        # CTC inputs: log_probs (seq_len, batch, num_classes) and lengths (batch,)
        ctc_inputs = {
            'log_probs': ctc_logits.permute(1, 0, 2),  # (seq_len, batch, num_classes)
