...
def PadMask(padded_input, input_lengths):
    ...
    # Create a range of indices [0, 1, ..., T-1] for each batch element
    indices = torch.arange(T, device=padded_input.device).unsqueeze(0)  # (1, T)
    mask = indices >= input_lengths.unsqueeze(1)  # (N, T)
    return mask
